...
const BotBubble = ({ text }: { text: string }) => (
  <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex items-end gap-2 mb-3">
    <img src={eerikImage} alt="Eerik" className="w-7 h-7 rounded-full object-cover shrink-0" />
...
const ChatPriceCalculator = () => {
  ...
  const startChat = useCallback(async () => {
    ...
    setStepUI(null);
    await addBotMessage(
      "Moikka! 👋 Oletko miettinyt, mitä kotisi maalaus tai katon pinnoitus voisi maksaa? Kokeile helppoa laskuriamme.",
    );
    ...
    setStepUI({
      ...
      options: [
        { label: "Talon maalaus", value: "maalaus", imageBase: "moderni-tumma-puutalo-julkisivumaalaus-valmis" },
        {
          ...
          value: "pinnoitus",
          imageBase: "puhdas-tiilenpunainen-tiilikatto-suojakasittelyn-jalkeen",
        },
      ],
    });
    setCurrentStep(1);
  ...
  const advanceFlow = useCallback(
    async (step: number, selectedPath: ServicePath) => {
      setStepUI(null);
      ...
      if (selectedPath === "maalaus") {
        switch (step) {
          case 2:
            await addBotMessage(
              "Hienoa! Lähdetään liikkeelle talon koosta. Kuinka monta pohjaneliötä talossasi on suunnilleen? Pelkkä numero riittää.",
            );
          ...
          case 5:
            await addBotMessage(
              "Hyvä tietää. Varmistetaan vielä pari käytännön asiaa: onhan kohteessa saatavilla vettä (vesiposti) ja sähköä (tavallinen pistorasia)?",
            );
            setStepUI({
              kind: "options",
              options: [
          ...
          case 7:
            await addBotMessage(
              "Kiitos tiedoista! Laitan laskimen raksuttamaan... 🔢 Saisinko tähän väliin nimesi ja puhelinnumerosi, niin voimme palata asiaan tarkemmin?",
            );
        ...
        switch (step) {
          case 2:
            await addBotMessage(
              "Hienoa! Lähdetään liikkeelle katon koosta. Mikä on katon arvioitu pinta-ala neliöissä? (Huom: katto on suurempi kuin talon pohja).",
            );
  ...
  const handleContactSubmit = useCallback(async () => {
    ...
    if (path === "maalaus") {
      ...
      const peelingLabels: Record<string, string> = {
        none: "Ei hilseilyä",
        "1-2": "1–2 seinällä",
        "3+": "Yli 3 seinällä",
      };
      ...
      priceStr = `${p.min.toLocaleString("fi-FI")} – ${p.max.toLocaleString("fi-FI")} €`;
      const brokenLabels: Record<string, string> = {
        ei: "Ei rikkinäisiä",
        "1-5": "1–5 rikkinäistä",
        "5-20": "5–20 rikkinäistä",
      };
    ...
    try {
      ...
      toast({
        title: "Virhe",
        description: "Jokin meni pieleen. Yritä uudelleen tai soita meille.",
        variant: "destructive",
      });
    } finally {
        ...
        >
          ...
          <div className="text-center mb-6">
            ...
            <p className="text-foreground/80 text-base md:text-lg leading-relaxed">
              Vastaa muutamaan kysymykseen ja saat <strong className="text-foreground">alustavan hinta-arvion</strong>{" "}
              heti.
            </p>
          </div>
